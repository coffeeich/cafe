  ...
  External: class External
    ...
    @require: (libs, callback) ->
      ...
      for dir, files of libs
        ...
        for file in files
          [file, search] = unless typeof file is "function" then file.split("?") else [file(), null]

          unless file is null
            script = "#{dir}/#{file}.js#{if search then '?' + search else ''}"

            loader = loader.script(script).wait()

      @current = loader.wait(callback if callback)

      return (callback) => @wait(callback)

    @wait: (callback) ->
      @current.wait(callback) if callback
...
# More information: http://labjs.com
...
External.loader.setGlobalDefaults(AllowDuplicates: no, UseLocalXHR: no)