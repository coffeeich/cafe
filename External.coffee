  ...
  External: class External
    ...
    @require: (libs, callback) ->
      ...
      for dir, files of libs
        ...
        for file in files
          loader = loader.script("#{dir}/#{file}.js")

      @loader = loader.wait(callback if callback)

      return (callback) => @wait(callback)

    @wait: (callback) ->
      @loader.wait(callback) if callback
...
External.loader.setGlobalDefaults(AllowDuplicates: no)